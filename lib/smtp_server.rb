...
class SMTPServer < GenericServer
  ...
  def process(client, command, full_data)
    ...
    # $log.info "SMTP Mailbox #{Store.instance.messages}"
  end
  ...
  def append_data(client, full_data)
    if full_data.gsub(/[\r\n]/, '') == '.'
      ...
      respond(client, 250)
      $log.info "Received mail from #{get_client_data(client, :from)} with recipient #{get_client_data(client, :to)}"
    else
      client_data[client.object_id][:data] << full_data
    end
  end
  ...
  def set_client_data(client, key, value)
    client_data[client.object_id] = {} unless client_data.include?(client.object_id)
    client_data[client.object_id][key] = value
  end

  # Retreive key from local client store
  def get_client_data(client, key)
    client_data[client.object_id][key] if client_data.include?(client.object_id)
  end
